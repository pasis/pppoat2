...
static void tp_xmpp_handlers_register(struct tp_xmpp_ctx *ctx)
{
	/* Message handler */
				   ...
				   void * const          userdata)
{
	...
	if (item == NULL)
		goto quit;
	xmpp_stanza_set_name(item, "name");
	...
	if (item == NULL)
		goto quit;
	xmpp_stanza_set_name(item, "version");
	xmpp_stanza_add_child(query, item);
	xmpp_stanza_release(item);
